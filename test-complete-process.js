// Script de test complet du processus d'achat de liens
...
async function testCompleteProcess() {
  console.log('🚀 Test complet du processus d\'achat de liens');
  console.log('==============================================\n');
...
async function step2_CreateWebsite() {
  ...
  const websiteData = {
    title: 'Site Test Éditeur',
    description: 'Site de test pour les liens',
    url: 'https://site-test-editeur.com',
    category: 'blog',
    niche: 'tech',
    ...
    contact_info: {
      email: 'contact@site-test-editeur.com',
      phone: '+212600000000'
    },
    slug: 'site-test-editeur',
    status: 'active',
...
async function step3_CreateLinkListing() {
  ...
  const linkListingData = {
    ...
    title: 'Lien Test - Article Tech',
    description: 'Lien dofollow dans un article sur la technologie',
    target_url: 'https://site-test-editeur.com/article-tech',
...
async function step4_CreateCampaign() {
  ...
  const campaignData = {
    name: 'Campagne Test - Tech',
    urls: ['https://site-annonceur-test.com'],
    language: 'Français',
    budget: 1000,
    status: 'draft'
  };
...
async function step5_GetRecommendations() {
  ...
  // Simuler la fonction getLinkRecommendations
  const { data: linkListings } = await supabase
    .from('link_listings')
    .select(`
      *,
      website:websites(*)
...
async function step6_CreatePurchaseRequest() {
  ...
  const purchaseRequestData = {
    ...
    target_url: 'https://site-annonceur-test.com',
    anchor_text: 'technologie innovante',
    message: 'Demande d\'achat de lien pour notre site tech',
    proposed_price: 300,
...
async function step7_AcceptRequest() {
  ...
  const { error } = await supabase
    ...
    .update({
      status: 'accepted',
      placed_url: 'https://site-test-editeur.com/article-tech#lien-place',
