// Test simplifié du processus d'achat de liens
...
// Fonction pour créer des comptes de test avec des emails plus courts
async function createTestAccounts() {
  console.log('👥 Création des comptes de test...');
  
  const uniqueId = Math.random().toString(36).substring(2, 8); // ID plus court
  
  // Créer l'annonceur
  const { data: advertiserAuth, error: advertiserError } = await supabaseAdmin.auth.signUp({
    email: `adv${uniqueId}@test.com`,
    password: 'password123',
    options: {
      data: {
        full_name: `Annonceur ${uniqueId}`,
        role: 'advertiser'
      }
    }
  });
  ...
  const { data: publisherAuth, error: publisherError } = await supabaseAdmin.auth.signUp({
    email: `pub${uniqueId}@test.com`,
    password: 'password123',
    options: {
      data: {
        full_name: `Éditeur ${uniqueId}`,
        role: 'publisher'
      }
    }
  });
...
async function createWebsite(publisherId, uniqueId) {
  ...
  const { data: website, error } = await supabaseAdmin
    ...
    .insert([{
      ...
      slug: `site-${uniqueId}`,
      description: `Site de test ${uniqueId}`,
      category: 'technology',
      domain_authority: 45,
...
async function createLinkListing(websiteId, uniqueId) {
  ...
  const { data: linkListing, error } = await supabaseAdmin
    ...
    .insert([{
      ...
      url: `https://site-${uniqueId}.com/article-${uniqueId}`,
      slug: `article-${uniqueId}`,
      description: `Article de test ${uniqueId}`,
      category: 'technology',
      price: 250,
      currency: 'MAD',
      is_available: true
...
async function testCompleteProcess() {
  console.log('🚀 Début du test simplifié du processus d\'achat de liens\n');
