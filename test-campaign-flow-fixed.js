// Test du flux complet campagne → demande d'achat (corrigé)
...
async function testCampaignFlowFixed() {
  console.log('🚀 Test du flux complet campagne → demande d\'achat (corrigé)\n');
  ...
  try {
    ...
    const { data: campaign, error: campaignError } = await supabaseAdmin
      ...
      .insert([{
        ...
        name: `Campagne Test ${uniqueId}`,
        urls: [`https://site-test-${uniqueId}.com`],
        language: 'Français',
        status: 'draft',
        budget: 1000
      }])
    ...
    const { data: website, error: websiteError } = await supabaseAdmin
      ...
      .insert([{
        ...
        title: `Site Campagne ${uniqueId}`,
        description: `Site pour test de campagne ${uniqueId}`,
        url: `https://site-campagne-${uniqueId}.com`,
        category: 'tech',
        niche: 'tech',
        owner_status: 'professionnel',
        metrics: { domain_authority: 50, monthly_traffic: 20000 },
        contact_info: { email: `contact@site-${uniqueId}.com` },
        slug: `site-campagne-${uniqueId}`,
        status: 'active',
    ...
    const { data: listing, error: listingError } = await supabaseAdmin
      ...
      .insert([{
        ...
        user_id: publisher.id,
        title: `Article Campagne ${uniqueId}`,
        description: `Article pour test de campagne ${uniqueId}`,
        target_url: `https://site-campagne-${uniqueId}.com/article-${uniqueId}`,
        anchor_text: 'test campagne',
        link_type: 'dofollow',
        position: 'content',
        price: 300,
        currency: 'MAD',
        ...
        status: 'active',
        slug: `article-campagne-${uniqueId}`
      }])
      .select()
      .single();
    ...
    // 5. Créer une demande d'achat AVEC campaign_id (valeurs courtes)
    ...
    const { data: purchaseRequest, error: requestError } = await supabaseAdmin
      ...
      .insert([{
        ...
        anchor_text: 'test campagne',
        message: `Demande depuis campagne ${campaign.name}`,
        proposed_price: 350,
        proposed_duration: 12,
        status: 'pending', // Valeur courte
    ...
    const { error: acceptError } = await supabaseAdmin
      .from('link_purchase_requests')
      .update({
        status: 'accepted', // Valeur courte
        editor_response: 'Demande acceptée depuis la campagne !',
        response_date: new Date().toISOString(),
        placed_url: `https://site-campagne-${uniqueId}.com/article-principal`,
