// Test minimal du flux campagne → demande d'achat
...
async function testCampaignMinimal() {
  console.log('🚀 Test minimal du flux campagne → demande d\'achat\n');
  ...
  try {
    ...
    const uniqueId = Math.random().toString(36).substring(2, 6); // Plus court
    ...
    // 5. Créer une demande d'achat AVEC campaign_id (valeurs très courtes)
    ...
    const { data: purchaseRequest, error: requestError } = await supabaseAdmin
      ...
      .insert([{
        ...
        anchor_text: 'test', // Très court
        message: 'Test', // Très court
        proposed_price: 350,
