// Test complet du flux campagne → demande d'achat après correction
...
async function testCampaignFixed() {
  console.log('🚀 Test complet du flux campagne → demande d\'achat après correction\n');
  ...
  try {
    ...
    // 5. Créer une demande d'achat AVEC campaign_id
    ...
    // 6. Vérifier que le statut de la campagne a été mis à jour par le trigger
    ...
    if (campaignUpdateError) {
      ...
      if (updatedCampaign.status === 'pending_editor_approval') {
        console.log('🎉 Le trigger automatique fonctionne parfaitement !');
      } else {
        console.log('⚠️  Le trigger n\'a pas mis à jour le statut comme attendu');
      }
    }
    ...
    const { error: acceptError } = await supabaseAdmin
      ...
      .update({
        status: 'accepted',
        editor_response: 'Demande acceptée depuis la campagne !',
        response_date: new Date().toISOString(),
        placed_url: `https://site-${uniqueId}.com/article-principal`,
        placed_at: new Date().toISOString()
      })
      .eq('id', purchaseRequest.id);

    if (acceptError) {
      throw new Error(`Erreur acceptation: ${acceptError.message}`);
    }
    ...
    const platformFee = 350 * 0.10; // 10%
    const publisherAmount = 350 - platformFee;
    ...
    await supabaseAdmin
      .from('users')
      .update({ balance: advertiser.balance - 350 })
      .eq('id', advertiser.id);
    ...
    console.log(`   - Montant: 350 MAD`);
    console.log(`   - Commission: ${platformFee} MAD`);
    console.log(`   - Éditeur: ${publisherAmount} MAD`);

    // 9. Vérifier les soldes finaux
    console.log('\n💳 Vérification des soldes finaux...');
    const { data: finalAdvertiser } = await supabaseAdmin
      .from('users')
      .select('balance')
      .eq('id', advertiser.id)
      .single();

    const { data: finalPublisher } = await supabaseAdmin
      .from('users')
      .select('balance')
      .eq('id', publisher.id)
      .single();

    console.log(`💰 Solde annonceur final: ${finalAdvertiser.balance} MAD`);
    console.log(`💰 Solde éditeur final: ${finalPublisher.balance} MAD`);
    ...
    console.log(`🎯 Trigger automatique: ${updatedCampaign.status === 'pending_editor_approval' ? '✅' : '❌'}`);

    // 11. Nettoyer les données de test
    console.log('\n🧹 Nettoyage des données de test...');
    await supabaseAdmin.from('link_purchase_transactions').delete().eq('id', transaction.id);
    ...
    // Remettre les soldes d'origine
    await supabaseAdmin.from('users').update({ balance: advertiser.balance }).eq('id', advertiser.id);
    await supabaseAdmin.from('users').update({ balance: publisher.balance }).eq('id', publisher.id);
