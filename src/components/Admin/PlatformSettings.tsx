...
const PlatformSettings: React.FC = () => {
  const [settings, setSettings] = React.useState<PlatformSettings>({
    ...
    admin_notification_email: 'admin@authority.ma',
  ...
  React.useEffect(() => {
    trackPageView('/admin/settings', 'Paramètres Plateforme | Authority.ma');
    loadSettings();
  }, []);
  ...
  const loadSettings = async () => {
    try {
      ...
      // Dans un vrai projet, on chargerait depuis une table settings
  ...
  const saveSettings = async () => {
    try {
      ...
      // Dans un vrai projet, on sauvegarderait dans une table settings
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Configurez les paramètres généraux d'Authority.ma
            </p>
          </div>
