// Script de test simplifié du processus d'achat de liens
...
async function testSimplifiedProcess() {
  console.log('🚀 Test simplifié du processus d\'achat de liens');
  console.log('===============================================\n');
  ...
  try {
    ...
    await step2_CheckTableStructure();
    
    // Étape 3: Création d'un site web par l'éditeur
    await step3_CreateWebsite();
    
    // Étape 4: Création d'une annonce de lien par l'éditeur
    await step4_CreateLinkListing();
    
    // Étape 5: Création d'une demande d'achat
    await step5_CreatePurchaseRequest();
    
    // Étape 6: Acceptation par l'éditeur
    await step6_AcceptRequest();
    
    // Étape 7: Traitement du paiement
    await step7_ProcessPayment();
    
    // Étape 8: Vérification des résultats
    await step8_VerifyResults();
...
async function step2_CheckTableStructure() {
  console.log('\n2️⃣ Vérification de la structure des tables...');
  
  // Vérifier la structure de link_purchase_requests
  ...
  if (requestsError) {
    console.log('⚠️  Erreur accès link_purchase_requests:', requestsError.message);
  } else {
    console.log('✅ Table link_purchase_requests accessible');
    if (requests && requests.length > 0) {
      console.log('📋 Colonnes disponibles:', Object.keys(requests[0]));
    }
  }
  
  // Vérifier la structure de link_purchase_transactions
  ...
  if (transactionsError) {
    console.log('⚠️  Erreur accès link_purchase_transactions:', transactionsError.message);
  } else {
    console.log('✅ Table link_purchase_transactions accessible');
...
async function step3_CreateWebsite() {
  console.log('\n3️⃣ Création d\'un site web par l\'éditeur...');
...
async function step4_CreateLinkListing() {
  console.log('\n4️⃣ Création d\'une annonce de lien par l\'éditeur...');
...
async function step5_CreatePurchaseRequest() {
  console.log('\n5️⃣ Création d\'une demande d\'achat...');
  
  // Créer la demande d'achat sans campaign_id
...
async function step6_AcceptRequest() {
  console.log('\n6️⃣ Acceptation de la demande par l\'éditeur...');
...
async function step8_VerifyResults() {
  console.log('\n8️⃣ Vérification des résultats...');
